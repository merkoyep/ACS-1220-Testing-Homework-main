...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_in(self):
        ...
        self.assertIn('To Kill a Mockingbird', response_text)
        self.assertIn('Harper Lee', response_text)
        publish_date = date(1960, 7, 11).strftime('%B %d, %Y')
        self.assertIn(publish_date, response_text)
    ...
    def test_create_author(self):
        ...
        post_data = {
            'name': 'Shakespear',
            'biography': 'Writer of many classics',
        }
        res = self.app.post('/create_author', data=post_data)

        # Verify that the author was updated in the database
        created_author = Author.query.filter_by(name='Shakespear').first()
        self.assertIsNotNone(created_author)
        self.assertEqual(created_author.name, 'Shakespear')
    ...
    def test_profile_page(self):
        ...
        response_text = res.get_data(as_text=True)
        self.assertIn('me1\'s favorite books', response_text)
    ...
    def test_favorite_book(self):
        ...
        user_me1 = User.query.filter_by(username='me1').first()
        favorite_book_ids = [book.id for book in user_me1.favorite_books]
        self.assertIn(1, favorite_book_ids)
    ...
    def test_unfavorite_book(self):
        ...
        user_me1 = User.query.filter_by(username='me1').first()
        favorite_book_ids = [book.id for book in user_me1.favorite_books]
        self.assertNotIn(1, favorite_book_ids)
